...
if [ -d ./build_mg_proto-double ]; 
then 
  rm -rf ./build_mg_proto-double
fi


mkdir  ./build_mg_proto-double
cd ./build_mg_proto-double

CXX=${PK_CXX} CC=${PK_CC} CXXFLAGS="${PK_CXXFLAGS} -g" cmake \
    -G"Eclipse CDT4 - Unix Makefiles" \
    -DMG_USE_KOKKOS=FALSE \
    -DMG_USE_QPHIX=TRUE \
    -DQPHIX_DIR=${INSTALLDIR}/qphix_${PK_QPHIX_ISA}-double \
    ...
    -DCMAKE_INSTALL_PREFIX=${INSTALLDIR}/mg_proto-double \
